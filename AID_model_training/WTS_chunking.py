...
def annotation_chunker_no_duplicates(
    kaleidoscope_df, chunk_length, include_no_bird=False, bird=None
):
    """
    ...
    kaleidoscope_df["FILEPATH"] = kaleidoscope_df.loc[:, ["FOLDER", "IN FILE"]].sum(
        axis=1
    )
...
def extact_split(df, chunk_len=3):
    def gen_list(a, b, chunk):
        ...
        multiplist = [i for i in range(
            math.ceil(a), math.ceil(b)) if i % chunk == 0]
        ...
        if multiplist:
            ...
            multiplist = [
                [multiplist[i], multiplist[i + 1]] for i in range(len(multiplist) - 1)
            ]
        return multiplist
    ...
    df["SPLIT"] = df.apply(
        lambda x: gen_list(x["OFFSET"], x["END_TIME"], chunk_len), axis=1
    )
    df_split = df.copy()
    ...
    df_split["END_TIME"] = df_split["END_TIME"].fillna(
        df_split["OFFSET"] + df_split["DURATION"]
    )
