...
def create_chunked_data():
    ...
    original_labeled_df["IN FILE"] = \
            original_labeled_df["IN FILE"].str.replace("_"," ")
...
def update_names(chunked_df):
    ...
    chunked_df[['FILE NAMES', 'FILE EXTENSIONS']] = \
            chunked_df['IN FILE'].str.split(pat='.', n=1, expand=True)
...
def load_clip(clip_path, verbose = False):
    try:
        signal, sample_rate = librosa.load(str(clip_path), sr=None, mono=True)       # Librosa uses range of 0-1, model needs range of -32768 to 32768
        signal = signal * 32768
